...
var SkuArraviePage = React.createClass({
 _refresh:function() {
    ...
    var url = DataAPI.sku_arravie_list+'?id='+id+'&code='+storeCode;
  ...
  const rightButtonConfig = {
    ...
    handler: () => this.props.navigator.push({
      ...
      <View style={styles.container}>
        {header}
          <PTRView
        onRefresh={this._refresh}>
      <View style={{backgroundColor:'#FFFFFF',height:Dimensions.get('window').height-100,justifyContent:'center',alignItems:'center'}}><Text style={{color:'#a9a9a9',fontSize:18,textAlign:'center'}}>暂无产品到货信息</Text></View>
       <Modal style={styles.modal} visible={this.state.visible} >            
                 <GiftedSpinner/>
           </Modal>  
           </PTRView>
	...
	skuArravieItemDetail: function (skuArravieItem){
	   	this.props.navigator.push({
	      id: 'SkuArravieDetail',
	      title: skuArravieItem.fromUser,
	      skuArravieItem: skuArravieItem,
	    });
	},

	renderRow: function(skuArravieItem) {
    var orderDetail=skuArravieItem.orderDetail[0];
		return (		  
			<TouchableHighlight underlayColor={'#f3f3f2'} >
        <View style={styles.rowCl}>
          <View style={styles.rowContainer}>
            <Text style={styles.rowCount}>
              <Icon name='cube' size={30} color='#142EB6' />
            </Text>
            <View style={styles.rowDetailsContainer}>
              <Text style={styles.rowTitle}>
                {skuArravieItem.dataTime}
              </Text>
