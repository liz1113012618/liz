   ...
   _refresh:function() {
	...
	async componentDidMount() {
      ...
      visible:true,
		...
		var url = DataAPI.member_advise+'?id='+id+'&code='+storeCode;
		fetch(url)
		 .then(res => res.json())
    ...
    handler: () => this.props.navigator.pop(),
      ...
      <View style={styles.container}>
        {header}
          <PTRView
        onRefresh={this._refresh}>
        <View style={{backgroundColor:'#FFFFFF',height:Dimensions.get('window').height-100,justifyContent:'center',alignItems:'center'}}><Text style={{color:'#a9a9a9',fontSize:18,textAlign:'center'}}>暂无建议</Text></View>
      <Modal
                style={styles.modal}
                visible={this.state.visible} >            
                    <GiftedSpinner/>
