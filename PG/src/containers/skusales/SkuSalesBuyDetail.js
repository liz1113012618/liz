...
var SkuSalesBuyDetail = React.createClass({
onPress: function () {
...
async processDataSubmit(){
  this.setState({visible:true});
   var customerId=this.props.customerId;
    var obj = eval('(' + customerId + ')');
    this.setState({customerId:obj});
      var url = DataAPI.scaneSaleOut;
   ...
   renderInputView: function() {
	...
	return (
     ...
     <Icon.Button name="save" backgroundColor="#142EB6" onPress={this.takePicture}>
            <Text style={{fontFamily: 'Arial', color:'white', fontSize: 15}}>扫码出货</Text>
         </Icon.Button>   
	         </View>
			<View style={styles.bottomview}>  	       
	        <Icon.Button name="save" backgroundColor="#142EB6" onPress={this.confirmReceipt}>
            <Text style={{fontFamily: 'Arial', color:'white', fontSize: 15}}>确定</Text>
         </Icon.Button>
	        </View>
...
module.exports = SkuSalesBuyDetail;
