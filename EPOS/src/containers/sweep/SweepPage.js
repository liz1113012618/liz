...
var SweepPage = React.createClass({
   _refresh:function() {
	...
	async componentDidMount() {
      ...
      visible:true,
		...
		var url = DataAPI.sample_list+'?id='+id+'&code='+storeCode;
		fetch(url)
		 .then(res => res.json())
    ...
    handler: () => this.props.navigator.pop(),
      ...
      <View style={styles.container}>
        {header}
          <PTRView
        onRefresh={this._refresh}>
        <View style={{backgroundColor:'#FFFFFF',height:Dimensions.get('window').height-100,justifyContent:'center',alignItems:'center'}}><Text style={{color:'#a9a9a9',fontSize:18,textAlign:'center'}}>暂无样品到货信息</Text></View>
      <Modal style={styles.modal} visible={this.state.visible} >            
                 <GiftedSpinner/>
           </Modal>
           </PTRView>
				...
				<View style={styles.rowCl}>
					<View style={styles.rowContainer}>
						<Text style={styles.rowCount}>
							<Icon name='truck' size={30} color='#142EB6' />
						</Text>
